...
class Stop:
    ...
    def totem(self) -> list:
        ...
        self.totem_result = []
        try:
            html_totem = ajax.post("https://www.divia.fr/bus-tram?type=479",
                 headers={
                 ...
                 oncomplete=self.totem_after_req, blocking=True
                 )
        finally:
            return self.totem_result

    def totem_after_req(self, result):
        try:
            html_totem = result.text
            two = False
            ...
            if (now - two_minutes) > time_1_today:
                self.totem_result.append(time_1_today + one_day)
            else:
                self.totem_result.append(time_1_today)
            if two:
                ...
                if (now - two_minutes) > time_2_today:
                    self.totem_result.append(time_2_today + one_day)
                else:
                    self.totem_result.append(time_2_today)
        except:
            pass
