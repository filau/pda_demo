...
class DiviaAPI:
    def __init__(self):
        raw_network = ajax.get("https://bo-api.divia.fr/api/reseau/type/json", oncomplete=self.init_req_complete, blocking=True)
    
    def init_req_complete(self, raw_network):
        self.network = loads(raw_network.text)
